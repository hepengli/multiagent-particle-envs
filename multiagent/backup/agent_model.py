...
class AgentModel(tf.Module):
    ...
    def compute_losses(self, ob, ac, atarg, estimates, multipliers, comm):
        ...
        neglogratio = pd.neglogp(ac) - old_pd.neglogp(ac)
        syncerr = comm * neglogratio - estimates
    ...
    def compute_jjvp(self, flat_tangents, ob, ac, atarg):
        ...
        with tf.GradientTape() as tape:
            ...
            pd, _ = self.pi.pdtype.pdfromlatent(policy_latent)
            neglogratio = pd.neglogp(ac) - old_pd.neglogp(ac)
            jvpr = tf.reduce_mean(jvp * neglogratio)
    ...
    def compute_vjp(self, ob, ac, atarg, estimates, multipliers, comm):
        with tf.GradientTape() as tape:
            ...
            policy_latent = self.pi.policy_network(ob)
            pd, _ = self.pi.pdtype.pdfromlatent(policy_latent)
            entbonus = self.ent_coef * (tf.exp(old_pd.logp(ac)) * (old_pd.logp(ac) + 1.))
            neglogratio = pd.neglogp(ac) - old_pd.neglogp(ac)
            v = - atarg + self.rho * entbonus - comm * multipliers + self.rho * comm * estimates
            vpr = tf.reduce_mean(v * neglogratio)
    ...
    def info_to_exchange(self, ob, ac, nb):
        ...
        return neglogratio, self.multipliers[nb]

    def exchange(self, ob, ac, nb_neglogratio, nb_multipliers, nb):
        old_policy_latent = self.oldpi.policy_network(ob)
        old_pd, _ = self.oldpi.pdtype.pdfromlatent(old_policy_latent)
        policy_latent = self.pi.policy_network(ob)
        pd, _ = self.pi.pdtype.pdfromlatent(policy_latent)
        ...
        v = 0.5 * (self.multipliers[nb] + nb_multipliers) + \
            0.5 * self.rho * (comm * neglogratio + (-comm) * nb_neglogratio)
        self.estimates[nb] = np.array((1.0/self.rho) * (self.multipliers[nb] - v) + comm * neglogratio).copy()
